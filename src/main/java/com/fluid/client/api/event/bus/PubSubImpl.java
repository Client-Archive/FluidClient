...
final class PubSubImpl<Event> implements PubSub<Event> {
    ...
    public <T extends Event> void subscribe(Class<T> event, Listener<T> listener) {
        ...
        this.subscriberMap.computeIfAbsent(GLOBAL_LISTENERS_BACKING_OBJECT, key -> new ArrayList<>()).add(new TypedListener(event, (Listener<Event>) listener));
    }
        ...
        for (Method method : subscriber.getClass().getDeclaredMethods()) {
            ...
            if (method.isAnnotationPresent(Listen.class)) {
                ...
                if (!Modifier.isPublic(mm)) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@Listen` must be public.", method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                // check is non-static
                if (Modifier.isStatic(mm)) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@Listen` must be non-static.", method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                // check is non-abstract
                if (Modifier.isAbstract(mm)) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@Listen` must be non-abstract.", method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                // check is non-native
                if (Modifier.isNative(mm)) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@Listen` must be non-native.", method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                // check method signature
                if (method.getReturnType() != void.class) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@Listen` must return `void`.", method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                ...
                if (paramTypes.length != 1) {
                    this.error(String.format("ERROR: %s#%s: methods marked with `@Listen` must have only a single parameter.", method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                ...
                try {
                    ...
                    CallSite site = LambdaMetafactory.metafactory(LOOKUP, "invoke", factoryType, LISTENER_INVOKE_TYPE, handle, MethodType.methodType(void.class, event));
                    ...
                    this.error(String.format("ERROR: %s#%s: the parameter of methods marked `@Listen` must be a subclass of the PubSub<Event> `Event` type parameter.", method.getDeclaringClass().getSimpleName(), method.getName()));
                } catch (Throwable e) {
    ...
    public void unsubscribe(Object subscriber) {
        ...
        this.subscriberMap.values().forEach(typedListeners -> typedListeners.forEach(typedListener -> this.eventTypeListenerCache.computeIfAbsent(typedListener.type, key -> new ArrayList<>()).add(typedListener.listener)));
    }
